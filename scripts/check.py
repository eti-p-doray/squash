...
def main(argv):
    ...
    for case in dataset:
        ...
        generated_file = '%s_%s_%s' % ('win64', case['new'], case['file'])

        old_path = os.path.join(indir, old_file)
        new_path = os.path.join(indir, new_file)
        patch_path = os.path.join(outdir, patch_file)
        generated_path = os.path.join(outdir, generated_file)

        if not os.path.isfile(generated_path):
          if not os.path.isfile(patch_path):
            utility.generate_patch(zucchini, old_path, new_path, patch_path)
          utility.apply_patch(zucchini, old_path, patch_path, generated_path)

        logging.info('files match: %r', filecmp.cmp(new_path, generated_path))
